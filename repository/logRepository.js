...
const ResourceConfig = Object.freeze({
    USERS: { name: 'USERS', expirationDays: 30 },
    TICKETS: { name: 'TICKETS', expirationDays: 90 },
    // Add more resource types and their expiration days here
});

async function storeAuditLog(resourceType, resourceId, action, description, createdBy, metadata = {}) {
    try {
        ...
        console.log(`Audit log inserted with _id: ${result.insertedId}`);
...
const getAuditLogs = async (resourceType, resourceId, action) => {
    ...
    const logs = await auditLogsCollection.find(query).sort({ created_at: -1 }).toArray();
    return logs;
};
module.exports.storeAuditLog = storeAuditLog;
