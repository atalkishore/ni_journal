 ...
 */
async function getFromCache(asyncCallback, redisCacheKey, expirationSeconds = 3600) {
    ...
    try {
        ...
        if (!cacheResult) {
            ...
            cacheResult && await redisClient.set(redisCacheKey, JSON.stringify(cacheResult), { 'EX': expirationSeconds });
        } else {
        ...
        try {
            ...
            console.log(`${redisCacheKey} db not available`);
        }
    }
...
async function cacheBreaker(redisCacheKey) {
    // const redisCacheKey = `${featureName}:${cacheKey}`;
    console.log(`Breaking cache for ${redisCacheKey}`);

    try {
        // Delete the cached data from Redis
        await redisClient.del(redisCacheKey);
        console.log(`Redis: ${redisCacheKey} cache successfully broken.`);
    } catch (error) {
        console.log(`Redis: Error breaking Redis cache for ${redisCacheKey}:`, error);
        LOGGER.error(`Redis: Error breaking Redis cache for ${redisCacheKey}:`, error);
    }
}
...
async function cacheBreakAndFill(redisCacheKey, defaultValue) {
    console.log(`Redis: Breaking Redis cache and filling data for ${redisCacheKey}`);
    ...
    try {
        ...
        console.log(`Redis: Error breaking cache and filling data for ${redisCacheKey}:`, error);
        LOGGER.error(`Redis: Error breaking cache and filling data for ${redisCacheKey}:`, error);
    }
}
