...
let countOpenTickets = async function (userId) {
    const _db = await mongodb;
    return await _db.collection('support_tickets').countDocuments({ userId: userId, status: 'open' });
}

// Method to create a support ticket
let createTicket = async function (ticketData) {
    const _db = await mongodb;
...
async function getPaginatedTickets(page, limit) {
    ...
    const tickets = await ticketsCollection.find({ status: { $eq: 'open' } })
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit)
        .toArray();
    ...
    const result = await ticketsCollection.updateOne(
        { _id: new ObjectId(ticketId) },
        {
            $set: {
                ...
                ttl_at: moment.utc().add(90, 'days').toDate()
            },
        }
    );
...
async function getTicketById(ticketId) {
    try {
        ...
        const usersCollection = _db.collection('users'); // Users collection (assuming this is where your users are stored)

        // MongoDB aggregation to join tickets with user details
        const ticket = await ticketsCollection.aggregate([
...
module.exports.countOpenTickets = countOpenTickets;
module.exports.createTicket = createTicket;
module.exports.getTicketsByUserId = getTicketsByUserId;
module.exports.getPaginatedTickets = getPaginatedTickets;
module.exports.updateTicketReply = updateTicketReply;
