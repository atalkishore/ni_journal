...
const { countOpenTickets, createTicket, getTicketsByUserId } = require("../repository/userRepository");
...
const renderSettingPage = async (res, message, formdata) => {
    ...
    res.render('authentication/settings', { message, fname: formdata?.name, femail: formdata?.email, ...commonVariables, });
};

router.get('/settings', connectEnsureLogin.ensureLoggedIn('/auth/login'), asyncMiddleware(async function (req, res) {
    renderSettingPage(res, { text: "", type: "" }, null)

}));

// Step 1: Define a route
router.post('/settings', connectEnsureLogin.ensureLoggedIn('/auth/login'), asyncMiddleware(async (req, res) => {
    try {
        ...
        if (newPassword?.length < 8) {
            return renderSettingPage(res, { text: "Password must be at least 8 characters long", type: "danger" }, req.body);
        }
        // Validate input data
        if (!email || !currentPassword || !newPassword || !confirmPassword) {
            return renderSettingPage(res, { text: 'All fields are required', type: 'danger' }, req.body);
        }

        if (currentPassword == newPassword) {
            return renderSettingPage(res, { text: 'old and new password cannot be same', type: 'danger' }, req.body);
        }

        if (newPassword !== confirmPassword) {
            return renderSettingPage(res, { text: 'New password and confirm password must match', type: 'danger' }, req.body);
        }

        // Fetch user from database
        const user = await _db.collection('users').findOne({ email });
        if (!user) {
            return renderSettingPage(res, { text: 'User not found', type: 'danger' }, req.body);
        }

        // Check if current password is correct
        const isMatch = await bcrypt.compare(currentPassword, user.password);
        if (!isMatch) {
            return renderSettingPage(res, { text: 'Invalid current password', type: 'danger' }, req.body);
        } else {
        ...
        return renderSettingPage(res, { text: 'Server error', type: 'warning' }, req.body);
...
// profile page
router.get('/profile', connectEnsureLogin.ensureLoggedIn('/auth/login'), asyncMiddleware(async function (req, res) {
    const _db = await mongodb;
    const existingUser = await _db.collection('users').findOne({ email: req.user?.email });
    const commonVariables = { menu: '', ...seoHeadTagValues("") };
...
router.post('/profile', connectEnsureLogin.ensureLoggedIn('/auth/login'), asyncMiddleware(async (req, res) => {
    try {
        ...
        const existingUser = await _db.collection('users').findOne({ email: email });

        if (existingUser) {
            const filter = { email: email };
            const update = { $set: { name, phone, updated_at: moment.utc().toDate() } };
            // const options = { upsert: true };
            const result = await _db.collection('users').findOneAndUpdate(filter, update, { returnDocument: 'after' });
            if (result) {
                req.user.name = name;
...
const renderForgotPassPage = async (res, message, formdata) => {
    const commonVariables = { menu: '', ...seoHeadTagValues("") };
    res.render('authentication/forgot-password', { message, fname: formdata?.name, femail: formdata?.email, ...commonVariables, });
};

router.get('/forgot-password', asyncMiddleware(async function (req, res) {
...
router.post('/forgot-password', asyncMiddleware(async (req, res) => {
    try {
        ...
        if (!user.email_verified) {
            req.flash('error', `<p class='mb-3'><strong>Please verify your email.</strong></p> To resend verification link <a href="/auth/send-verify-link/${email}">click here</a>`);
            return res.redirect('/user/forgot-password');
        }
        ...
        if (validRequests.length >= 3) {
            req.flash('error', 'You can only request a password reset three times in an hour. Please check your spam folder for the email.');
            return res.redirect('/user/forgot-password');
        }
        ...
        if (validDailyRequests.length >= 5) {
            req.flash('error', 'You have reached the limit of password reset requests today. Please try again tomorrow or check your spam folder for the email.');
            return res.redirect('/user/forgot-password');
        }
        ...
        if (validRequests.length > 0) {
            ...
            if (timeSinceLastRequest < 2) {
                req.flash('error', 'Password reset request is already in process. Please check your spam folder for the email.');
                return res.redirect('/user/forgot-password');
            }
        }
        ...
        const expirationDate = moment().add(1, 'hour').tz('Asia/Kolkata').valueOf();
        ...
        transporter.sendTemplateMail('PASSWORD_RESET', user.email, { resetUrl: `${process.env.HOST}/auth/reset-password/${resetToken}` });
...
router.get('/support', connectEnsureLogin.ensureLoggedIn('/auth/login'), asyncMiddleware(async (req, res) => {
    // Render the view located at authentication/support
    res.render('authentication/support', { menu: '', ...seoHeadTagValues("Contact-Us") });
}));

router.post('/api/support-tickets', connectEnsureLogin.ensureLoggedIn('/auth/login'), asyncMiddleware(async (req, res) => {
    let { subject, message } = req.body;
    const MAX_SUBJECT_LENGTH = 50, MAX_MESSAGE_LENGTH = 1000;
    ...
    try {
        ...
        if (openIssuesCount >= 3) {
            return res.status(400).json({ message: 'You can only have 3 open tickets at a time.' });
        }
        ...
        return res.status(201).json({ message: 'Your support request has been submitted.', ticketId });
    } catch (error) {
        console.error(error);
        return res.status(500).json({ message: 'An error occurred while submitting your request.' });
    }
}));

router.get('/api/support-tickets', connectEnsureLogin.ensureLoggedIn('/auth/login'), asyncMiddleware(async (req, res) => {
    ...
    try {
        ...
        console.error(error);
        return res.status(500).json({ message: 'An error occurred while fetching your support tickets.' });
    }
}));
